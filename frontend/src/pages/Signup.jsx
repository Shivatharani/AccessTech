  ...
  const signup = async () => {
    ...
    try {
      await API.post("/auth/signup", {
        ...
        confirm_password: form.confirm_password, language: form.language, level: form.level
      })
        ...
        <div className="relative z-10 space-y-8">
          <div className="space-y-4">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-white/15 dark:bg-black/20 rounded-full">
              ...
              <span className="text-white/80 dark:text-white/90 text-sm font-semibold">Free to get started</span>
            </div>
        ...
        <div className="w-full max-w-[420px] py-8">
          ...
          <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); signup(); }}>
            {[
              ...
              <div key={key}>
                <label className="block text-xs font-black uppercase tracking-widest mb-2 text-teal-400 dark:text-teal-600">{label}</label>
                <input
                  type={type}
              ...
              <div key={key}>
                ...
                <div className="relative">
                  <input
                    ...
                    onChange={(e) => setForm({ ...form, [key]: e.target.value })}
                  />
                  <button type="button" className="absolute right-3 top-3 p-1 transition-colors text-teal-400 hover:text-teal-600 dark:text-teal-600 dark:hover:text-teal-400" onClick={toggle}>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <div key={key}>
                  <label className="block text-xs font-black uppercase tracking-widest mb-2 text-teal-400 dark:text-teal-600">{label}</label>
                  <div className="relative">
                    <Icon size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-teal-400 dark:text-teal-600" />
                    <select
          ...
          <p className="text-center mt-6 text-sm font-medium text-teal-500 dark:text-teal-400">
            ...
            <Link to="/login" className="font-bold hover:underline ml-2 text-teal-700 dark:text-teal-500">
              {t('login')}
            </Link>
          </p>
        </div>
